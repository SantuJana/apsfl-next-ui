...
async function VMSEvents({ host, day }: { host: string, day: string }) {
  const date = new Date()
  ...
  const counts = await getEventCounts(host, `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate() - (dayObj[day] || 0)}`, 'ivms')
  ...
  return (
    <div>
        ...
        <CountsTable counts={counts} type={'ivms'} host={host} day={day} />
    </div>
  )
}
